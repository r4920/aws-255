...
async function seedUser () {
  try {
    ...
    userToBeInserted = {
      'password':'kNd18HaGr7FA8c7',
      'username':'Kirk.McClure',
      'isDeleted':false,
      'isActive':true,
      'userType':authConstant.USER_TYPES.User
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let user = await dbService.findOneAndUpdateDocument(User, { 'username':'Kirk.McClure' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
    userToBeInserted = {
      'password':'2jer1fKlYOimPmH',
      'username':'Dayana_Strosin',
      'isDeleted':false,
      'isActive':true,
      'userType':authConstant.USER_TYPES.Admin
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.findOneAndUpdateDocument(User, { 'username':'Dayana_Strosin' }, userToBeInserted,  {
      upsert: true,
      new: true
    });
